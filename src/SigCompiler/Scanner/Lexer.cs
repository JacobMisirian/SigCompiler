    ...
    public class Lexer
    {
                ...
                else
                {
                    switch ((char)peekChar()) {
                        ...
                        case '}':
                            ...
                            break;
                        case '[':
                            add(TokenType.OpenBrace, ((char)readChar()).ToString());
                            break;
                        case ']':
                            add(TokenType.CloseBrace, ((char)readChar()).ToString());
                            break;
                        case ',':
        ...
        private void scanNumber()
        {
            ...
            if(bname != "")
            {
                ...
                {
                    throw new CompilerException(location, "Invalid {0} number {1}!", bname, final);
                }
            }
            else
            {
                ...
                catch
                {
                    ...
                    {
                        throw new CompilerException(location, "Invalid number: {0}!", final);
                    }
                }
            }
        }
        ...
        private char scanEscapeCode(char escape)
        {
            switch (escape) {
                ...
                case '#':
                    return '#';
                default:
                    throw new CompilerException(location, "Unknown escape sequence {0}!", escape);
            }
        }
        ...
        private int readChar() {
            if (peekChar() == '\n')
                location = new SourceLocation(location.File, location.Row + 1, 0);
            else
                location = new SourceLocation(location.File, location.Row, location.Column + 1);
